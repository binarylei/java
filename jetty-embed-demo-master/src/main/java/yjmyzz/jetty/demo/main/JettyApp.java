...
public class JettyApp {
    ...
    private static void unzipSelf() {
        ...
        if (selfPath.endsWith(".jar")) {
            ...
            try {
                logger.info("正在将\n" + selfPath + "\n解压至\n" + param.get(WEB_DIR));
                String path = param.get(WEB_DIR);
                System.out.println("runtime path = "+path);
                JarUtils.unJar(selfPath,path);
            } catch (Exception e) {
                logger.error("解压web内容失败!", e);
            }
        } else {
            // IDE环境
        	System.out.println("dev path = "+selfPath);
            param.put(WEB_DIR, selfPath);
           //param.put(WEB_DIR, "D:\\Documents\\Downloads\\jetty-embed-demo-master\\web\\");
        }
